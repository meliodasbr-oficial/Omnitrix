const characters = [
    ...
    { name: "AlienX", health: 80, attacks: [
        ...
        { name: "Controle Total", cost: 0, dice: "???" }
    ]}
];
...
function loadSoundSettings() {
    ...
    selectSound.muted = savedMuted;
    transformSound.muted = savedMuted;
    muteToggle.textContent = savedMuted ? "🔇" : "🔊";
    
    bgMusic.volume = savedVolume;
    selectSound.volume = savedVolume;
    transformSound.volume = savedVolume;
...
muteToggle.addEventListener("click", () => {
    ...
    if (!muted && bgMusic.paused) {
        bgMusic.play().catch(error => console.error("Erro ao tentar reproduzir o áudio:", error));
    }
});
...
document.getElementById("select").addEventListener("click", selectCharacter);
